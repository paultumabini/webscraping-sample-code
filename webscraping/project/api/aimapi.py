...
class AimApiData:
    ...
    def render_api_data(cls, aimdata):
        ...
        for dealer in aimdata:
            try:
                status = AimDealer.objects.annotate(id_str=Cast('dealer_id', output_field=CharField())).get(id_str=dealer.get('id'))
        ...
        print(
            'WEBSCRAPE:',
            [[d.account, d.dealer_id, d.dealer_name] for d in AimDealer.objects.all().order_by('account')],
        )
        print('Webscrape Dealer Account Status successfully updated!')
